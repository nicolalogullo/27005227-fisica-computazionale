/*
  ...
  Esattamente identico a sis_scal.cpp, solo, per mostrare l'effetto della imprecisione negli
  arrotondamenti nei calcoli, nel caso in cui la matrice NON sia diagonalmente dominante,
  rimuovo la condizione: a_ij = 1 se i = j.
  La matrice viene costruita come:
  a_ij = 1.0/(1.0+i+j)

  Provare, ad esempio, il caso: n = 3 (dim=3), n = 8 (dim=8) e n = 20 (dim=20) per vedere come 
  l'approssimazione peggiora all'aumentare di n. Nota che per n=50 il programma va in overflow,
  arrivandoci abbastanza improvvisamente (n=47 da' ancora un risultato abbastanza preciso!)
