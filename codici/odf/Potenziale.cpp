/*
  ...
  con:
     ...
     x = r        -> coordinata radiale
     Y(x) = V(r)  -> Potenziale elettrostatico
     h(x) = h(r) = rho_0/epsilon_0      se 0 <= r < R; 
                 = 0                    se r > R
     f(x) = f(r) = 2/r
     g(x) = 0

  che risulta nell'equazione di Poisson in coordinate sferiche:

     d^2 V(r) / dr^2 + ( 2 / r ) * d V(r) / dr = - rho_0 / epsilon_0
      
  con condizioni al bordo:

     V(r=0)  = 0;
     V(r=R_ext) = rho_0 R^3 / ( 3 eps_0 R_ext ) - rho_0 R^2 / ( 2 eps_0 )

  La soluzione esatta dell'equazione e':

       V(r) = - rho_0 r^2 / ( 6 eps_0 )                             se 0 <= r <= R;
       V(r) = rho_0 R^3 / ( 3 eps_0 r ) - rho_0 R^2 / ( 2 eps_0 )   se r > R.
...
// Parametri del problema specifico

double R0 = 1.0, Rext = 5.0, rho0 = 1.0, eps0 = 1.0;
...
// Y(a) e Y(b) noti:
  ...
  double Ya  = 0.0;
  double Yb  = rho0 * R0 * R0 * R0 / ( 3.0 * eps0 * Rext ) - rho0 * R0 * R0 / ( 2.0 * eps0 );
...
double h( double x )
{
  ...
  if ( x < R0 ) val = - rho0 / eps0;
  if ( x == R0 ) val = - 0.5 * rho0 / eps0;
  if ( x > R0 ) val = 0.0;
  return( val );
}
...
int main()
{
  ...
  b = Rext;    // Inserire l'estremo destro dell'intervallo di definizione
